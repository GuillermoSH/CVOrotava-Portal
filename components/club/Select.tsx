...
export function Select({
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key.length === 1 && !e.ctrlKey && !e.metaKey && !e.altKey) {
        ...
        const idx = flat.findIndex((o) => o.label.toLocaleLowerCase("es").startsWith(start));
  ...
  const triggerSize =
    size === "compact" ? "min-h-9 px-3 py-2 text-sm" : "min-h-11 px-3 py-2.5 text-sm";

  return (
    <>
      <div className={cn("relative", size === "form" && "w-full", className)}>
        <button
          ref={triggerRef}
          id={id}
          type="button"
          ...
          className={cn(
            "flex w-full cursor-pointer items-center justify-between gap-2 rounded-[var(--radius-md)] border text-left transition-colors focus:outline-none focus:ring-2 focus:ring-ring disabled:cursor-not-allowed disabled:opacity-50",
            triggerSize,
            error
              ? "border-destructive focus:ring-destructive/30"
              : "border-border bg-[var(--club-surface-2)] text-foreground focus:border-brand focus:ring-brand/25",
          ...
          <AnimatePresence>
            {open && pos && (
              <motion.div
                ...
                className="z-[110] overflow-y-auto rounded-[var(--radius-md)] border border-border bg-[var(--club-drawer-bg)] py-1 shadow-xl shadow-black/25"
              >
                {menuRows.map((row) => {
                  if (row.type === "group") {
                    return (
  ...
  return (
    <button
      ...
      className={cn(
        ...
        active
          ? "bg-[var(--club-brand-soft)] text-brand"
